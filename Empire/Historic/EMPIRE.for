...
900	TRY=TRY+1
	IFILE(10)="A"
	IFILE(10)=IFILE(10)+RND(10)	'CURRENTLY SICityLocation MAPS, ALLOW 4 EXTRA
...
C
	MONKEY=0
	EnemyCount(1)=0
	IF (DebugLevel.EQ.1) TYPE 999
999	FORMAT(" ARMY CODES")
C 
C START ARMY MOVE LOOP
C
	DO 4200 Y=1,UpperIndex(9)
	Z6=UnitLocation(CArmyStart+Y)
...
200	Z7=Z6
	...
	P=0
	AB=ReferenceMap(Z6)		'SET AB=WHAT IS SHOWING WHERE THE ARMY IS
	AC=0
	IF ((AB.NE."a").AND.(AB.NE."t").AND.(AB.NE."X")) GOTO 3700
C 
C AGE UnitTimer
C
	IF ((UnitTimer(Y).LE.100).OR.(UnitTimer(Y).GT.1000)) UnitTimer(Y)=UnitTimer(Y)-1
	IF ((UnitTimer(Y).LT.0).OR.(UnitTimer(Y).EQ.1000)) UnitTimer(Y)=0
	IF (AB.EQ."a") GOTO 300
	IF (AB.EQ."X") THEN
	  DO 250 I=1,UpperIndex(13)
	  IF (UnitLocation(CTransportStart+I).EQ.Z6) GOTO 270
250	  CONTINUE
	  GOTO 300
	ENDIF
270	IF (ARMJMP(Z6,UnitTimer(Y)).EQ.0) GOTO 4150
C 
C MOVE SELECTION
C
300	IFO=UnitAction(Y)
	ILA=UnitActionTarget(Y)
C 
...
CIFO=1: MOVE TOWARDS TargetCity CITY
...
C 
500	IF (ReferenceMap(ILA).EQ."X") GOTO 800	'CITY HAS BEEN CAPTURED
	GOTO 1600			'MOVE
C 
...
700	IF (ILA.GT.100) GOTO 800	'INVALID VALUE FOR ILA
	IF (UnitAction(ILA+CTransportStart-1500).GE.6) GOTO 1200
	IF (UnitLocation(ILA+CTransportStart).EQ.0) GOTO 1200			'TT SUNK
	IF (UnitHits(ILA+CHitsTransportStart).LT.3) GOTO 1200		'TT DAMAGED
	GOTO 1700
C 
C SELECT A NEW CODE
C
...
C
	...
	IA=RND(EnemyCount(10))+1
	IB=IA+EnemyCount(10)-1
	DO 1000 IC=IA,IB
	I=IC
	IF (I.GT.EnemyCount(10)) I=I-EnemyCount(10)
	IF (TargetCity(I).EQ.0) GOTO 1000
	IF (IDIST(Z6,TargetCity(I)).GT.14) GOTO 1000
	MOVE=PATH(Z6,TargetCity(I),DIR,EnemyArmyTerrain,FLAG)
	NPATH=NPATH+1
...
1050	IF (Sightings(10,11).NE.0) Sightings(10,11)=0
	...
	IF (Sightings(I,TEMP).EQ.0) GOTO 1100
	TEMP=Sightings(I,TEMP)
	MOVE=PATH(Z6,TEMP,DIR,EnemyArmyTerrain,FLAG)
	NPATH=NPATH+1
...
C
1200	IF (UnitTimer(Y).NE.0) GOTO 1400		'INELIGIBLE TO GET ON A TT
	IA=RND(UpperIndex(13))+1			'**
	DO 1300 IC=IA,IA+UpperIndex(13)
	I=IC
	IF (I.GT.UpperIndex(13)) I=I-UpperIndex(13)
	IF (UnitLocation(CTransportStart+I).EQ.0) GOTO 1300		'TT DOESN"T EXIST
	IF (UnitHits(CHitsTransportStart+I).LT.3) GOTO 1300	'DAMAGED, I.E. UNSUITABLE
	IF (IABS(UnitAction(CTransportStart+I-1500)).GE.6) GOTO 1300
	IF (IDIST(Z6,UnitLocation(CTransportStart+I)).GT.20) GOTO 1300	'TOO FAR AWAY
	MOVE=PATH(Z6,UnitLocation(CTransportStart+I),DIR,EnemyArmyTerrain,FLAG)
	NPATH=NPATH+1
	IF (FLAG.EQ.0) GOTO 1300			'CAN"T GET TO IT
	MOVE=MOV(Z6,UnitLocation(CTransportStart+I))
	IFO=3
	ILA=I
	UnitActionTarget(CTransportStart+I-1500)=Y
	GOTO 1800
1300	CONTINUE
C 
...
1500	MOVE=ILA
	I1=ICORR(MOVE-DIR*3)
	IF (ReferenceMap(Z6+MoveOffset(I1+1)).NE."+") MOVE=I1	'**
	GOTO 1800

1600	MOVE=PATH(Z6,ILA,DIR,EnemyArmyTerrain,FLAG)
	NPATH=NPATH+1
	IF (FLAG.EQ.0) GOTO 1400
	GOTO 1800

1700	MOVE=PATH(Z6,UnitLocation(ILA+CTransportStart),DIR,EnemyArmyTerrain,FLAG)
	NPATH=NPATH+1
C 
1800	DO 2300 I=0,7*DIR,DIR
	MOVE1=ICORR(MOVE+I)
	LOC=Z6+MoveOffset(MOVE1+1)			'**
	AC=ReferenceMap(LOC)
	IF (AC.NE."t") GOTO 2200
	IF (IFO.EQ.3) IFO=0
	IF (UnitTimer(Y).NE.0) GOTO 2300
	NUMARM=0
	DO 1900 IZ=CTransportStart+1,UpperIndex(13)+CTransportStart
1900	IF (UnitLocation(IZ).EQ.LOC) GOTO 2000
2000	IF (UnitHits(CHitsTransportStart-CTransportStart+IZ).LT.3) GOTO 2300
	DO 2100 IY=CArmyStart+1,UpperIndex(9)+CArmyStart
	IF (UnitLocation(IY).EQ.LOC) NUMARM=NUMARM+1
...
2400	IF (IFO.EQ.0) ILA=IABS(MOVE1)
	UnitAction(Y)=IFO
	UnitActionTarget(Y)=ILA
	IF (DebugLevel.EQ.1) TYPE 998,IFO,ILA
998	FORMAT(1X,7I,3CityLocation)
	Z6=Z6+MoveOffset(MOVE1+1)			'**
C 
	AC=ReferenceMap(Z6)
	IF (AB.NE."t") GOTO 2500
	IF (AC.EQ."t") GOTO 3600
	UnitAction(Y)=0
	UnitActionTarget(Y)=0
	UnitTimer(Y)=1020
	GOTO 2600
2500	IF (MasterMap(Z7).NE."*") ReferenceMap(Z7)=MasterMap(Z7)
	IF (AC.NE."t") GOTO 2600
	UnitTimer(Y)=100
	GOTO 3600
2600	IF (AC.EQ."+") GOTO 3500
	IF ((AC.EQ."X").OR.(AC.EQ.".")) GOTO 3700
	IF (MasterMap(Z6).NE."*") GOTO 3400
	...
	IF (IDIST(UnitLocation(CArmyStart+I),Z6).GE.ID) GOTO 2650
	MOVE=PATH(UnitLocation(CArmyStart+I),Z6,1,EnemyArmyTerrain,FLAG)
	NPATH=NPATH+1
	IF (FLAG.EQ.0) GOTO 2650		'CAN"T GET TO IT
	ID=IDIST(UnitLocation(CArmyStart+I),Z6)
	IY=I
2650	CONTINUE
	IF (ID.LT.10) THEN
	IFO=1
	...
	DO 2700 I=1,70
2700	IF (TargetCity(I).EQ.Z6) TargetCity(I)=0
	DO 2800 I=1,UpperIndex(9)
2800	IF (UnitAction(I).NE.1.OR.UnitActionTarget(I).NE.Z6) GOTO 2900
	UnitAction(I)=0
	UnitActionTarget(I)=0
2900	DO 3000 I=1,100
3000	IF (CityLocation(I).EQ.Z6) GOTO 3100
3100	CityOwner(I)=2
	CityProduction(I)=0
	IF (((AC.EQ."O").OR.(UnitTimer(Y).GT.0)).AND.(EDGER(Z6).LT.8))
	...
	CALL SENSOR(Z6)
	GOTO 3700
3200	ReferenceMap(Z6)="X"
	GOTO 3700
3300	UnitTimer(Y)=100
	GOTO 3600
3400	H1=1
...
997	FORMAT(1H+,/," ERROR: ATTACKED ",A1,4I,1X)
	...
	IF (ReferenceMap(Z6).EQ.".") GOTO 3700
3500	ReferenceMap(Z6)="a"
3600	UnitLocation(CArmyStart+Y)=Z6
	IF (P.EQ.1) CALL SENSOR(Z6)
	GOTO 4100
3700	UnitLocation(CArmyStart+Y)=0
	IF (AC.NE."X") GOTO 3900
	DO 3800 I=1,70
3800	IF (CityLocation(I).EQ.Z6) CityProduction(I)=0
3900	IF (P.EQ.1) CALL SENSOR(Z6)
	IF (ReferenceMap(Z6).NE."O") GOTO 4000
	CALL LIB$SET_CURSOR(3,1)
	CALL STROUT("City at",10)
	CALL DECPRT(Z6)
...
4000	UnitAction(Y)=0
	...
	UnitTimer(Y)=0
4100	CALL SONAR(Z6)
4150	IF (UnitLocation(CArmyStart+Y).NE.0) EnemyCount(1)=EnemyCount(1)+1
4200	CONTINUE
	UpperIndex(9)=MONKEY
	RETURN
	END
...
C
	MONKEY=0
	EnemyCount(5)=0
	IF (DebugLevel.EQ.5) TYPE 999
999	FORMAT(" TROOP TRANSPORT CODES")
C 
	DO 2300 Y=1,UpperIndex(13)
	Z6=UnitLocation(CTransportStart+Y)
	IF (Z6.EQ.0) GOTO 2300
	MONKEY=Y
	DIR=MOD(Y,2)*2-1		'SET DIR TO 1 OR -1 CONSISTENTLY
	AB=ReferenceMap(Z6)
	H1=UnitHits(CHitsTransportStart+Y)
	IF (AB.EQ."X") H1=H1+1
	IF (H1.GT.3)    H1=3
C 
...
C
	NUMARM=0
	DO 100 I=1,UpperIndex(9)
100	IF (Z6.EQ.UnitLocation(CArmyStart+I)) NUMARM=NUMARM+1
	IF (NUMARM.GT.6) NUMARM=6		'MACityLocation # ARMIES = 6
C 
	ORIG=Z6
	DO 2200 ITURN=1,2
	P=0
...
C MOVE SELECTION
C
	IFO=UnitAction(Y+CTransportStart-1500)
	ILA=UnitActionTarget(Y+CTransportStart-1500)
C 
...
200	IFO=8
	...
	IF (ReferenceMap(ILA).EQ."X") GOTO 1300
250	ILA=IPORT(Z6)
	GOTO 1300
C 
...
C
	DO 400 I=1,70
	IF (TargetCity(I).NE.ILA) GOTO 400
	MOVE=0
...
400	CONTINUE
	...
	1 GOTO 1300
C
C	IT SEEMS THAT IT"S TargetCity IS NO LONGER ON THE HIT LIST,
...
500	IF (IFO.LE.2) GOTO 600
	...
	ENDIF
	GOTO 800			'SELECT A TargetCity
C
C SELECT AN ARMY PRODUCING CITY AND MOVE TOWARDS IT.
C PICK THE CLOSEST ONE.
C
600	IF (ILA.EQ.0.OR.ILA.GT.500) GOTO 650
	IF ((UnitAction(ILA).EQ.3).AND.(UnitLocation(CArmyStart+ILA).NE.0)) GOTO 1200
650	AFLAG=0
	ID=35
670	DO 700 I=1,70
	IF ((CityLocation(I).EQ.0).OR.(CityOwner(I).NE.2)) GOTO 700
	IF (EDGER(CityLocation(I)).EQ.0) GOTO 700
	IF ((AFLAG.EQ.0).AND.(CityProduction(I).NE.1)) GOTO 700
	IF (IDIST(Z6,CityLocation(I)).GE.ID) GOTO 700
	DO 680 J=1,UpperIndex(13)
	IF (J.EQ.Y) GOTO 680
	IF (UnitActionTarget(J+CTransportStart-1500).NE.CityLocation(I)) GOTO 680
	IF (IDIST(UnitLocation(J+CTransportStart),CityLocation(I)).LE.2) GOTO 700
680	CONTINUE
	ID=IDIST(Z6,CityLocation(I))
	ILA=CityLocation(I)
700	CONTINUE
	...
	GOTO 670
C 
C PERFORM TROOP TRANSPORT TO TargetCity CITY ASSIGNMENT
C
800	IF (EnemyCount(10).EQ.0) GOTO 1000
	TM=0
	DO 820 I=1,UpperIndex(13)
	IF (UnitLocation(CTransportStart+I).EQ.0) GOTO 820
	IF (I.EQ.Y) GOTO 810
	IF (UnitAction(CTransportStart-1500+I).EQ.8) GOTO 820
	IF (UnitAction(CTransportStart-1500+I).LE.3) GOTO 820
810	TM=TM+1
	IF (UnitAction(CTransportStart-1500+I).EQ.9) UnitAction(CTransportStart-1500+I)=0
	TTTC(TM,0)=I
820	CONTINUE
	CM=0
	DO 840 I=1,EnemyCount(10)
	IF (TargetCity(I).EQ.0) GOTO 840
	ILA=TargetCity(I)
	IF (EDGER(ILA).EQ.0) GOTO 840
	CM=CM+1
	TTTC(0,CM)=ILA
	TTTC(-1,CM)=-1
	IF (ReferenceMap(ILA).EQ."O") TTTC(-1,CM)=1
840	CONTINUE
	DO 850 I=1,TM
	DO 850 J=1,CM
	TTTC(I,J)=IDIST(UnitLocation(CTransportStart+TTTC(I,0)),TTTC(0,J))
850	CONTINUE
...
870	  CONTINUE
	ENDIF
	MOVE=PATH(UnitLocation(CTransportStart+TTTC(I,0)),TTTC(0,J),1,OKC,FLAG)
	IF (FLAG.EQ.0) THEN
	  TTTC(I,J)=1000
	  GOTO 880
	ENDIF
...
880	CONTINUE
	IF (MIN.NE.1000) THEN  'DON"T CHANGE FUNCTION IF DEST IS <3 FROM OLD?
	  UnitAction(CTransportStart-1500+TTTC(IR,0))=9
	  UnitActionTarget(CTransportStart-1500+TTTC(IR,0))=TTTC(0,IC)
	  CALL DIST(UnitLocation(CTransportStart+TTTC(IR,0)),TTTC(0,IC))
	  TTTC(IR,0)=0
	  TTTC(IR,IC)=1001
	  TTTC(-1,IC)=0
	  GOTO 860
	ENDIF
	IFO=UnitAction(CTransportStart-1500+Y)
	ILA=UnitActionTarget(CTransportStart-1500+Y)
	IF (EnemyCount(9)+EnemyCount(10).LE.38) THEN
...
C
1200	MOVE=0
	IF (IDIST(Z6,UnitLocation(CArmyStart+ILA)).EQ.1) GOTO 1600
	MOVE=MOV(Z6,UnitLocation(CArmyStart+ILA))
	GOTO 1500
1300	MOVE=PATH(Z6,ILA,DIR,OKC,FLAG)
	IF (FLAG.EQ.0) GOTO 1100
	GOTO 1500
1350	IF (EnemyCount(10).EQ.0) GOTO 1400
	IF (RND(100).LT.40) GOTO 800
1400	MOVE=ILA
1500	AGGR=-NUMARM
	IF ((EnemyCount(5).GT.10).AND.(NUMARM.EQ.0)) AGGR=AGGR+2
	EXPLOR=0
...
1600	UnitAction(CTransportStart-1500+Y)=IFO
	UnitActionTarget(CTransportStart-1500+Y)=ILA
	Z6=Z6+MoveOffset(MOVE+1)			'**
	IF (DebugLevel.EQ.5) TYPE 997, IFO,ILA
997	FORMAT(1X,I)
C 
	IF (MasterMap(Z7).NE."*") ReferenceMap(Z7)=MasterMap(Z7)
	IF (ReferenceMap(Z6).EQ.".") GOTO 1700
	IF (ReferenceMap(Z6).EQ."X") GOTO 1800
	IF ((ReferenceMap(Z6).EQ."+").OR.(MasterMap(Z6).EQ."*")) GOTO 1900
	AB=ReferenceMap(Z6)
	IF (DebugLevel.EQ.5) TYPE 996,AB		'FIX THIS CONDITIONAL, KLUDGED
996	FORMAT(" ATTACKING ",A1)
	IF (AB.EQ.".") GOTO 1700
	P=1
	H2=30
	OWN1="t"
	OWN2=ReferenceMap(Z6)
	CALL FIND(OWN2,Z6,Z8,H2)
	CALL FGHT(Z6,H1,H2,OWN1,OWN2)
	CALL FIND(OWN2,Z6,Z8,H2)
	IF (H1.LE.0) GOTO 1900
	IF (MasterMap(Z6).EQ."+") GOTO 1900
	UnitHits(CHitsTransportStart+Y)=H1
1700	ReferenceMap(Z6)="t"
1800	UnitLocation(CTransportStart+Y)=Z6
	UnitHits(CHitsTransportStart+Y)=H1
	IF (ITURN.EQ.1) EnemyCount(5)=EnemyCount(5)+1
	GOTO 2000
1900	UnitLocation(CTransportStart+Y)=0
	UnitHits(CHitsTransportStart+Y)=0
2000	N=0
	IF (P.EQ.1) CALL SENSOR(Z6)
	DO 2100 U=CArmyStart+1,CArmyStart+UpperIndex(9)
	IF (UnitLocation(U).NE.Z7) GOTO 2100
	IF (N+1.GT.H1*2) THEN
	 IF (ReferenceMap(Z7).NE."X") UnitLocation(U)=0
	 GOTO 2100
	ENDIF
	N=N+1
	UnitLocation(U)=Z6
...

	FUNCTION MOVCOR
	1	(IFO,ITURN,Z6,MOVE,IH1,AGGR,OWN1,EXPLOR,DIR,DEST,ORIG,HMACityLocation)
C
	INCLUDE "EMPIRE.INC/NOLIST"
C 
...
800	IF (EXPLOR.EQ.0) GOTO 1000		'**
	...
	ENDIF
	NEXP=0
	IF (EMAP(LOC1+MoveOffset(I1+1)).EQ." ") NEXP=1		'**
	IF (EMAP(LOC1+MoveOffset(ICORR(I1-1)+1)).EQ." ") NEXP=NEXP+1	'**
	IF (EMAP(LOC1+MoveOffset(ICORR(I1+1)+1)).EQ." ") NEXP=NEXP+1	'**
	IF (EMAP(LOC1+MoveOffset(ICORR(I1+2)+1)).EQ." ") NEXP=NEXP+1	'**
	IF (EMAP(LOC1+MoveOffset(ICORR(I1-2)+1)).EQ." ") NEXP=NEXP+1	'**
	IF (NEXP.EQ.5) GOTO 1200
	IF (NEXP.LE.EXPMACityLocation) GOTO 900
	EXPMACityLocation=NEXP
	I11=I1
900	CONTINUE
	I1=0
	IF (EXPMACityLocation.EQ.0) GOTO 1000
	I1=I11
	GOTO 1200
1000	I2=MOVE
...
900	IF (Z6.EQ.UnitLocation(ILA+2800)) GOTO 1100	'IF LANDED
	IF (UnitLocation(ILA+2800).EQ.0) GOTO 1100	'IF CARRIER DOESN"T EXIST
...
C
	...
	I1=Z6+ARROW(I)
	AB=ReferenceMap(I1)
	IF (AB.EQ.PlayerMap(I1)) GOTO 100
	PlayerMap(I1)=AB
	...
	1	.OR.(ICityLocation.LE.KI).OR.(ICityLocation.GT.KI+SWIDTH)) GOTO 100
	I1=I1-LINE-KI
	IF (GIGI.AND.(IBEFOR.EQ.-100)) CALL REGIS_ON
	IF (IBEFOR+1.NE.I1) CALL CURSOR(I1+300)
	IBEFOR=I1
	IF (.NOT.GIGI) GOTO 200
	color=7 'WHITE
	if (ab.eq."+") color=4
	if (ab.eq.".") color=5
	...
	if (ab.eq."X") color=2
	if (ab.eq."*") color=6
	type 995,"T(W(I0)(N0)(R)S[12])" ""
	CALL CURSOR(i1+300)	'POSITION CURSOR
	if (color.eq.4.or.color.eq.5) then
	if (ab.eq.".") ab=" "
	type 995,"T(W(I",48+COLOR,")(N1)S[12])"",ab,""T(S[9])"
	else
	type 995,"T(W(I",48+COLOR,")(N0)(R)S[9])"",ab,"""
	end if
	GOTO 100
995	format("+",$,100(A))
200	TYPE 999,AB
...
100	IF (MasterMap(Z6+MoveOffset(I+1)).NE.".") GOTO 200	'** NOT ALL SEA SURROUND
	RETURN
C 
200	IF (UnitTimerC.EQ.0) GOTO 400		'BEEN ON TROOP TRANSPORT
C						'FOR A LONG TIME
	DO 300 I=1,8
...
C	TYPE 999,N,EXPMACityLocation
C999	FORMAT(" N:",I2," EXPMACityLocation:",I2)
	IF (N.LE.EXPMACityLocation) GOTO 200
	PRIOR(4)=MOVE
	EXPMACityLocation=N
200	CONTINUE
C	TYPE 998
